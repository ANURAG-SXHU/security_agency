...
#         weekday = self.check_in_time.strftime("%A")
#         check_in_time_only = self.check_in_time.time()

#         # Find Deployment Sheet
#         deployment_sheet = frappe.get_all(
#             "Deployment Sheet",
#             filters={
#                 "week_start_date": ["<=", self.check_in_time.date()],
#                 "week_end_date": [">=", self.check_in_time.date()],
#             },
#             fields=["name"]
#         )

#         if not deployment_sheet:
#             frappe.throw("No Deployment Sheet found for the current week.")

#         # Find Deployment Line
#         deployment_line = frappe.get_all(
#             "Deployment Line",
#             filters={
#                 "parent": deployment_sheet[0].name,
#                 "guard": self.employee,
#                 "site": self.site,
#                 "day": weekday
#             },
...
#         if not deployment_line:
#             frappe.throw("No Deployment Line found for this guard/site/day.")

#         shift_name = deployment_line[0].shift
...
#         # Check if check_in_time falls within shift hours (including overnight)
#         is_within_shift = False
#         if shift_start < shift_end:
#             # Day shift
#             is_within_shift = shift_start <= check_in_time_only <= shift_end
#         else:
#             # Overnight shift
#             if check_in_time_only >= shift_start or check_in_time_only <= shift_end:
#                 is_within_shift = True

#         if is_within_shift:
#             self.shift_type = shift_name
#         else:
#             frappe.throw(f"Check-in time does not fall within shift hours for {shift_name}.")
